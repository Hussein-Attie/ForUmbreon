...
public class MSSceneController 
		...
		if (vehicleCode) {
			if (vehicleCode._vehicleState == MSVehicleController.ControlState.isPlayer) {
				...
				if (selectControls == MSSceneController.ControlType.windows) {
					...
					//cameras input
					if (controls.enable_switchingCameras_Input_key) {
						if (Input.GetKeyDown (controls.switchingCameras) && vehicleCode.indexCamera < (vehicleCode._cameras.vehicleCameras.Count - 1)) {
							vehicleCode.indexCamera++;
							vehicleCode.EnableCameras (vehicleCode.indexCamera);
						} else if (Input.GetKeyDown (controls.switchingCameras) && vehicleCode.indexCamera >= (vehicleCode._cameras.vehicleCameras.Count - 1)) {
							vehicleCode.indexCamera = 0;
							vehicleCode.EnableCameras (vehicleCode.indexCamera);
						}
					}
					...
					if (vehicleCode.youCanCall && controls.enable_startTheVehicle_Input_key) {
						if (vehicleCode.theEngineIsRunning) {
							...
							if (Input.GetKeyDown (controls.startTheVehicle) || (Mathf.Abs (verticalInput) > 0.5f && vehicleCode._vehicleSettings.turnOnWhenAccelerating)) {
	...
	void EnableOrDisableButtons(bool insideInCar){ //on Update
		if (selectControls == ControlType.mobileButton || selectControls == ControlType.mobileJoystick || selectControls == ControlType.mobileVolant) {
			...
			if (controls.switchingCamerasMobileButton) {
				if (vehicleCode._cameras.vehicleCameras.Count <= 1) {
			...
			if (selectControls == ControlType.mobileButton) {
				...
				if (vehicleCode.cameraTypeIndice == 1) { 
					if (joystickCamera) {
						if (vehicleCode._cameras.vehicleCameras.Count > 0) {
				...
				} else if (vehicleCode.cameraTypeIndice == 2) { 
					if (joystickCamera) {
						if (vehicleCode._cameras.vehicleCameras.Count > 0) {
							joystickCamera.gameObject.SetActive (insideInCar);
						} else {
	...
	void Mobile_SwitchingCamerasInputButton(){
		if (!error) {
			if (controls.enable_switchingCameras_Button_Mobile) {
				if (vehicleCode) {
					if (vehicleCode._vehicleState == MSVehicleController.ControlState.isPlayer) {
						if (vehicleCode.indexCamera < (vehicleCode._cameras.vehicleCameras.Count - 1)) {
							vehicleCode.indexCamera++;
							vehicleCode.EnableCameras (vehicleCode.indexCamera);
						} else if (vehicleCode.indexCamera >= (vehicleCode._cameras.vehicleCameras.Count - 1)) {
							vehicleCode.indexCamera = 0;
							vehicleCode.EnableCameras (vehicleCode.indexCamera);
						}
					}
